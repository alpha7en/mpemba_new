...
The master equation transforms into a matrix differential equation $\frac{d}{dt}\ketket{\rho} = L\ketket{\rho}$, where the matrix representation $L$ of the Liouvillian superoperator $\mathcal{L}$ \textcolor{magenta}{(Liouvillian is non-normal?$\mathcal{L}\mathcal{L}^{\dagger}\ne\mathcal{L}^{\dagger}\mathcal{L}$)} has dimensions $N^2 \times N^2$ and can be explicitly constructed by using the Kronecker product ($\otimes$). Separating the coherent and dissipative parts, we obtain the target matrix:
\begin{equation}
