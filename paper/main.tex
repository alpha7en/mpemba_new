...
To investigate the system's response to disorder, for every disorder realization we identify the slowest non-trivial mode $\lambda_1$ as the eigenvalue with the largest real part. Depending on the realization, this mode is either purely real or belongs to an oscillating complex-conjugate family ($\operatorname{Im}\lambda_1 \neq 0$) inherited from the degenerate multiplets of the regular lattice. All statistics are collected over 30 independent graph realizations for each $p$; the resulting relaxation rate is shown in Fig.~\ref{fig:modes_log_scale}(a).\
...
\caption{Statistics of the slowest non-trivial Liouvillian mode versus the rewiring probability $p$ (log scale) for a $10 \times 10$ lattice; for each realization the mode is identified by the largest $\operatorname{Re}\,\lambda$. \textbf{(a)} Mean relaxation rate $\operatorname{Re}(\lambda_1)$; the conventional Liouvillian relaxation gap is $-\operatorname{Re}(\lambda_1)$. \textbf{(b)} Fraction of realizations whose slowest mode is oscillating ($\operatorname{Im}\lambda_1 \neq 0$): the oscillating family inherited from the regular lattice loses its dominance with disorder and disappears for $p \gtrsim 2\times10^{-2}$. \textbf{(c)} Mean oscillation frequency $|\operatorname{Im}(\lambda_1)|$ over the oscillating subensemble. \textbf{(d)} Separation between the two slowest \emph{distinct} relaxation rates, $\Delta = \operatorname{Re}(\lambda_1) - \operatorname{Re}(\lambda_2)$; differences between individual modes are ill-defined inside degenerate multiplets. Error bars: $\pm 1$ standard deviation over 30 realizations in (a), (c), (d); binomial standard error in (b).}
\label{fig:modes_log_scale}
\end{figure}
We observe the following. First, for $p \lesssim 2\times10^{-2}$ the slowest mode of a given realization is predominantly a member of the oscillating family inherited from the regular lattice ($|\operatorname{Im}\lambda_1| \approx 0.24$); disorder progressively suppresses this family, so the fraction of oscillating realizations drops below one half at $p \approx 4\times10^{-3}$ and vanishes for $p \gtrsim 2\times10^{-2}$ (panel~b). Second, the mean relaxation rate is \textbf{non-monotonic}: $|\operatorname{Re}(\lambda_1)|$ first grows with disorder --- a mild ensemble-averaged \emph{acceleration} of relaxation peaking at $p \approx 0.03$ --- and then decreases toward the random-graph limit, corresponding to a \textbf{slowing down} of the slowest mode. The latter trend may appear \textbf{counter-intuitive}, since, on average, a small-world network relaxes to equilibrium faster; however, individual configurations hosting significantly slower localized modes emerge with increasing probability and dominate the ensemble average, which is accompanied by the sharp growth of the realization-to-realization scatter visible in panel~(a).
